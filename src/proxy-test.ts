...
async function main() {
  ...
  // ── Test 5: recordSpend — budget actually decrements ─────────────────────
  console.log("\n── Test 5: recordSpend() — budget decrements after execution ──");

  // Check budget before
  const beforeCheck = budgetEngine.check(
    "did:workload:payment-agent-01",
    "execute_wire_transfer",
    0
  );
  const remainingBefore = beforeCheck.remainingDailyUsd ?? 0;
  console.log("  Budget remaining before:", `$${remainingBefore.toFixed(2)}`);

  // Execute a $3,000 action
  const call5: McpToolCall = {
    ...call1,
    id: "req-005",
    params: { ...call1.params, _estimated_cost_usd: 3_000 },
  };
  await proxyA.forwardToolCall(call5, async () => ({
    transaction_id: "txn_stripe_005",
    status: "settled",
  }));

  // Wait briefly for fire-and-forget /executed to process
  await new Promise(r => setTimeout(r, 100));

  // Check budget after
  const afterCheck = budgetEngine.check(
    "did:workload:payment-agent-01",
    "execute_wire_transfer",
    0
  );
  const remainingAfter = afterCheck.remainingDailyUsd ?? 0;
  const spent = remainingBefore - remainingAfter;

  if (Math.abs(spent - 3_000) < 1) {
    console.log("  Budget remaining after: ", `$${remainingAfter.toFixed(2)}`);
    console.log(`✓ recordSpend() correct: $${spent.toFixed(2)} deducted`);
  } else {
    console.error(`✗ recordSpend() FAILED: expected $3000 deducted, got $${spent.toFixed(2)}`);
  }
